...
class Util:
    ...
    def get_from_json_file(filename):
        # greedy file is saved in rl_utils since it is used by both hri and training
        if filename == "greedy":
            current_file_dir = os.path.dirname(os.path.abspath(__file__))
            config_file_path = os.path.join(current_file_dir, "../../rl_utils/env/greedy.json")
            if not os.path.exists(config_file_path):
                raise FileNotFoundError(f"greedy.json file not found at: {config_file_path}")
