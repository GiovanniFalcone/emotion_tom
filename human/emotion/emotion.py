...
def send_emotion(emotion):
    url = "http://" + IP_ADDRESS + ":5000/emotion"
    ...
    try:
        ...
        print("Impossibile connettersi al server. Verifica che il server sia in esecuzione.")
    except requests.exceptions.HTTPError as err:
        print(f"Errore HTTP: {err}")
    except Exception as err:
        print(f"Si è verificato un errore: {err}")

print("*** Running emotion.py")
IP_ADDRESS = Util.get_from_json_file("config")['ip'] 

# global variable to track the emotion
last_emotion = ''
