...
val UniversalFallbackBehaviour = partialState {
    ...
    onNoResponse {
        furhat.say("Scusami, non ti sento. ")
        reentry()
    }
    onNoResponse() {
        noInput++
        when (noInput) {
            0 -> furhat.say("Scusami, non ti sento. ")
            else -> furhat.say("Hey, continuo a non sentire. ")
        }
        reentry()
    }
    onResponse {
        noMatch++
        when (noMatch) {
            0 -> furhat.say("Scusami, ma non ho capito. ")
            else -> furhat.say("Scusami, ma continuo a non capirti. ")
        }
        reentry()
    }
    // Override default error messages on internet failure
    onResponseFailed {
        furhat.say("Scusami, sto avendo problemi di connessione.")
        reentry()
    }
    onNetworkFailed {
        furhat.say("Scusami, ma sto riscontrando problemi di rete.")
        reentry()
    }
